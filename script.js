...
const quizQuestions = [
    { q: "在參加小組討論時，我感到非常放鬆。" },
    { q: "在小組討論中，我通常不擔心表達自己的意見。" },
    { q: "當我必須在小組討論中說話時，我會感到緊張和困擾。" },
    { q: "在小組討論中說話時，我感到很平靜。" },
    { q: "參與小組討論會讓我感到非常焦慮。" },
    { q: "在小組討論中，我感到非常自在。" },
    { q: "在參加會議時，我通常感到緊張。" },
    { q: "在會議中說話時，我感到非常放鬆。" },
    { q: "當我被要求在會議中表達意見時，我感到害怕。" },
    { q: "在會議中說話時，我感到很平靜。" },
    { q: "在會議中回答問題會讓我感到困擾。" },
    { q: "在會議中表達意見時，我感到非常放鬆。" },
    { q: "在與新認識的人交談時，我感到非常放鬆。" },
    { q: "在與他人交談時，我害怕表達自己的想法。" },
    { q: "當我與某人單獨交談時，我感到非常緊張。" },
    { q: "與他人交談時，我感到非常放鬆。" },
    { q: "在與他人對話時，我常感到緊張和焦慮。" },
    { q: "與他人交談時，我感到非常自在。" },
    { q: "我有信心面對群眾做公開演講。" },
...
function startQuestions() {
    const id = document.getElementById('quiz-player-id').value;
    const gender = document.getElementById('quiz-gender').value;
    if(!id || !gender) return alert("請完成身分登錄。");
...
// 警報動畫流程
async function triggerAlertSequence() {
    switchScene('scene-alert-anim');
    const subtitleEl = document.getElementById('alert-subtitles');
    
    // 背景震動效果
    document.body.style.animation = "screen-shake 0.2s infinite";

    const scripts = [
        { text: ">>> 系統初始化...", voice: "初始化中" },
        { text: "⚠️ 警告：偵測到非法外部存取 ⚠️", voice: "警告，偵測到非法外部存取" },
        { text: "中央伺服器室 封鎖啟動", voice: "中央伺服器室，封鎖啟動" },
        { text: "人員狀態確認：Dr. Aris Thorne [已離線]", voice: "人員已離線" },
        { text: "歡迎來到深淵，審查官。", voice: "歡迎來到深淵" }
    ];

    for (let msg of scripts) {
        subtitleEl.innerText = "";
        speak(msg.voice); // 語音
        await typeWriter(msg.text, subtitleEl); // 打字字幕
        await new Promise(r => setTimeout(r, 1200));
    }

    document.body.style.animation = "";
    switchScene('scene-survey'); // 進入原本的遊戲輸入名畫面
}
